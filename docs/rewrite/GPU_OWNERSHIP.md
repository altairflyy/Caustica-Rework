...
This document records the production ownership and lifetime rules observed at
AER-041. It is an audit of the current implementation, not a target design.
...
| Vanilla terrain section BLAS, AS backing, UV/material buffers | `RtTerrain` resident map through `RtSectionTable.SectionGeom`; `PreparedSection` owns resources until publication | `RtSectionBuilder.prepare(...)` and `RtAccel.prepareTerrainBlas(...)` | Async `Build` until publication; replaced resident generations use `latestGraphicsUse()` | Build scratch/upload are released in completion callbacks; stale never-published results use `retireUnpublished`; published geometry uses `DeferredDeletionQueue.retireAfterGraphics` | Async compute build, then graphics trace | Chunk publication/rebuild/eviction, material or terrain epoch, world reset, shutdown |
...
| LOD BLAS, backing, build scratch, UV/material buffers | `RtLodTerrain.BuildSession` owns new geometry until checkpoint/final publication; ownership then transfers to the published `Proxy` | `RtSectionBuilder.prepare(...)`, `RtAccel.prepareTerrainBlas(...)`, optional compaction | Async `Build` before publication; replaced `Proxy` uses `latestGraphicsUse()` | Scratch/upload and uncompacted sources are released after compute completion; aborted unpublished work is destroyed; replaced/cleared proxies retire through `DeferredDeletionQueue` with retained geometry excluded | Async compute build/compaction, then graphics trace | Provider revision/selection, progressive checkpoint/final replacement, retry/reset, shutdown |
...
| Block-entity BLAS/backing/packed geometry | `RtEntities.BeEntry` | `RtEntities` through persistent `RtAccel` build helpers and `RtContext` buffers | Per-entry `TrackedGraphicsUse`, marked by `RtEntities.markGraphicsUse(...)` | `retireBe(...)` removes ownership then submits the exact tracked token and grouped destroy callback to `DeferredDeletionQueue` | Graphics build/trace | Mesh hash change, eviction/unload, entity reset, teardown |
| Dynamic-entity BLAS/backing/geometry/refit scratch | One `RtEntities.EntitySlot` in the per-entity ring | `RtEntities` through `RtAccel.prepareUpdatableBlasBuild` and buffer factories | Per-slot `TrackedGraphicsUse`, marked after successful frame enqueue | Slot reuse waits for its tracked completion; eviction/replacement uses `retireEntitySlot(...)` through `DeferredDeletionQueue`; teardown is idle | Graphics build/refit and trace | Ring advance, topology/capacity change, stale entity eviction, teardown |
...
| Frame TLAS, AS backing, instance buffer, scratch | `RtAccel.TlasRing`, held by `RtComposite` | `RtAccel.prepareTlas/createTlasSlot` | Per-slot `TrackedGraphicsUse`; marked with the frame token and awaited before overwrite/resize | A completed slot may be destroyed immediately on capacity growth; all slots are destroyed by `TlasRing.destroy()` during idle teardown | Graphics build and ray tracing | Ring advance, instance-capacity growth, teardown |
...
Every critical resource family required by AER-041 has a declared production
