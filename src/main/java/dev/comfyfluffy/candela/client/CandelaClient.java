...
import dev.comfyfluffy.candela.rt.terrain.RtWorkerPool;
import net.fabricmc.api.ClientModInitializer;
import net.fabricmc.fabric.api.client.event.lifecycle.v1.ClientLifecycleEvents;
import net.fabricmc.fabric.api.client.event.lifecycle.v1.ClientTickEvents;
import net.fabricmc.fabric.api.client.rendering.v1.InvalidateRenderStateCallback;

public final class CandelaClient implements ClientModInitializer {
	private static boolean rtInitDone = false;

	@Override
	public void onInitializeClient() {
		CandelaMod.LOGGER.info("Candela client initialized");
			...
			if (rtInitDone) {
				...
				if (ctx != null) {
					...
					if (dev.comfyfluffy.candela.rt.pipeline.RtDlssFg.enabled()) {
						dev.comfyfluffy.candela.rt.pipeline.RtDlssFg.INSTANCE.probeAvailabilityOnce();
					}
				}
			}
		});
	...
	private static void shutdownRt() {
		...
		dev.comfyfluffy.candela.rt.pipeline.RtDlssFg.INSTANCE.destroy();
		if (ctx != null) {
			dev.comfyfluffy.candela.rt.RtFramePresenter.INSTANCE.destroy(ctx.device());
			dev.comfyfluffy.candela.rt.RtReflex.INSTANCE.destroy(ctx.device().vkDevice());
		}
		// Shut NGX down once, after every feature (RR + FG) has been released above.
		dev.comfyfluffy.candela.ngx.NgxRuntime.INSTANCE.shutdown();
		if (ctx != null) {
			ctx.destroy();
		}
		rtInitDone = false;
