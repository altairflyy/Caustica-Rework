...
public final class VanillaRenderController {
	...
	public void beginFrame(RenderTarget mainTarget) {
		...
		if (!Boolean.valueOf(this.rtActive).equals(this.lastLoggedRtActive)) {
			...
			CandelaMod.LOGGER.info("RT output mode: {}", this.rtActive ? "rt" : "vanilla");
		}
		...
		if (this.baseReady) {
			if (!this.loggedActive) {
				...
				CandelaMod.LOGGER.info("Vanilla world rendering cancellation active; using existing RT composite seam");
			}
		} else {
	...
	public boolean shouldCancelLevelRenderer(boolean waitingForRtPlayerSection) {
		...
		if (waitingForRtPlayerSection && !this.loggedWaitingForRtPlayerSection) {
			...
			CandelaMod.LOGGER.info("Keeping vanilla LevelRenderer canceled while waiting for RT player section residency");
		}
		return true;
	}
	...
	public void markRtPlayerSectionReady() {
		if (!this.loggedRtPlayerSectionReady) {
			...
			CandelaMod.LOGGER.info("Satisfied vanilla terrain-load callback from RT player section residency");
		}
	}
	...
	private void latchFailure(String reason) {
		if (!this.failureLatched) {
			CandelaMod.LOGGER.warn("Disabling vanilla world cancellation: {}", reason);
		}
	...
	private void logInactive(String reason) {
		...
		CandelaMod.LOGGER.info("Vanilla world cancellation inactive: {}", reason);
