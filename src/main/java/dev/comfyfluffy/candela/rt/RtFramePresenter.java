 ...
 * Gated by {@code candela.rt.fg} (default off).
 */
public final class RtFramePresenter {
    ...
    public void prepareExtraFrames(VulkanCommandEncoder enc, VulkanDevice device, long swapchain,
        ...
        try {
            ...
            failed = true;
            pendingCount = 0;
            CandelaMod.LOGGER.error("DLSS-FG present-record failed; frame generation disabled", t);
        }
    }
    ...
    public void flushPendingPresents(long swapchain, VkQueue presentQueue) {
        ...
        if (!failed && pendingCount != 0) {
            try (MemoryStack stack = MemoryStack.stackPush()) {
                ...
                failed = true;
                CandelaMod.LOGGER.error("DLSS-FG present failed; frame generation disabled", t);
            } finally {
                pendingCount = 0;
            }
        }
     ...
     * is genuinely receiving more frames. Logged only when {@code candela.rt.fg} is enabled.
     */
