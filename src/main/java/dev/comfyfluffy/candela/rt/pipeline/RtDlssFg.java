package dev.comfyfluffy.candela.rt.pipeline;
...
import dev.comfyfluffy.candela.ngx.NgxLibrary;
import dev.comfyfluffy.candela.ngx.NgxRuntime;
 ...
 * with the present-path refactor. Gated by {@code candela.rt.fg} (default off) and hardware/driver support.
 */
public final class RtDlssFg {
    public static final RtDlssFg INSTANCE = new RtDlssFg();

    public static boolean enabled() {
        return CandelaConfig.Rt.Fg.ENABLED.value();
    }
    ...
    public int effectiveMultiFrameCount() {
        int requested = CandelaConfig.Rt.Fg.MULTI_FRAME_COUNT.value();
    ...
    public void probeAvailabilityOnce() {
        ...
        if (!(((GpuDeviceAccessor) RenderSystem.getDevice()).candela$getBackend() instanceof VulkanDevice device)) {
            return;
        }
        ...
        lib = l;
        if (!l.hasDlssg()) {
            CandelaMod.LOGGER.warn("DLSS-FG: loaded ngxshim.dll has no DLSSG ABI — rebuild the shim "
                    + "(cmake --build native/ngx_shim/build --config Release)");
            return;
        }
        available = l.dlssgAvailable();
        multiFrameCountMax = l.dlssgMultiFrameCountMax();
        CandelaMod.LOGGER.info("DLSS Frame Generation available: {} (multi-frame max {})", available, multiFrameCountMax);
    }
    ...
    public boolean ensureFeature(long cmd, int width, int height, int renderWidth, int renderHeight, int backbufferFormat) {
        ...
        try {
            ...
            if (featureWidth != width || featureHeight != height
                ...
                featureBackbufferFormat = backbufferFormat;
                initialized = true;
                CandelaMod.LOGGER.info("DLSS-FG feature created: {}x{} (render {}x{}, backbuffer format {})",
                        width, height, renderWidth, renderHeight, backbufferFormat);
            }
            return true;
        } catch (Throwable t) {
            failed = true;
            CandelaMod.LOGGER.error("DLSS-FG setup failed; frame generation disabled", t);
            return false;
        }
    }
    ...
    public boolean evaluate(long cmd,
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            failed = true;
            CandelaMod.LOGGER.error("DLSS-FG evaluate failed; frame generation disabled", t);
            return false;
        }
    }
