...
public final class RtDlssRr {
    ...
    public static boolean enabled() {
        return CandelaConfig.Rt.DlssRr.ENABLED.value();
    }
    ...
    private static int renderPreset() {
        return CandelaConfig.Rt.DlssRr.PRESET.value();
    }

    public static int quality() {
        return CandelaConfig.Rt.DlssRr.QUALITY.value();
    }
    ...
    public boolean evaluate(long cmd, RtImage color, RtImage depth, RtImage motion,
        ...
        try {
            ...
            failed = true;
            CandelaMod.LOGGER.error("DLSS-RR evaluate failed; RT composite continues without it", t);
            return false;
        }
    }
    ...
    public boolean ensureFeature(long cmd, int renderWidth, int renderHeight, int displayWidth, int displayHeight) {
        ...
        try {
            ...
            if (featureRenderWidth != renderWidth || featureRenderHeight != renderHeight
                ...
                CandelaMod.LOGGER.info("DLSS-RR feature created: {}x{} -> {}x{} (quality {}, preset {})",
                        renderWidth, renderHeight, displayWidth, displayHeight, quality, preset);
            }
            return true;
        } catch (Throwable t) {
            failed = true;
            CandelaMod.LOGGER.error("DLSS-RR setup failed; RT composite continues without it", t);
            return false;
        }
    }
    ...
    private void ensureInitialized(VulkanDevice device) {
        ...
        if (!loggedAvailable) {
            ...
            CandelaMod.LOGGER.info("DLSS Ray Reconstruction available: {}", available);
        }
        if (!available) {
     ...
     * teardown ({@code NgxRuntime.shutdown()} in {@code CandelaClient.shutdownRt}), so FG can keep using NGX.
     */
    public void destroy() {
        if (((GpuDeviceAccessor) RenderSystem.getDevice()).candela$getBackend() instanceof VulkanDevice device) {
            releaseFeature(device);
