...
public final class RtExposure {
    ...
    private float manualExposureScale() {
        return CandelaConfig.Rt.Exposure.clampScale((float) Math.pow(2.0, manualEv()));
    }
    ...
    private void logOnce() {
        ...
        CandelaMod.LOGGER.info("RT display exposure: mode={}, exposure={}, tonemap=agx, DLSS-RR exposure=NGX auto",
                mode.configName, exposureText);
    }

    private static Mode mode() {
        return Mode.parse(CandelaConfig.Rt.Exposure.MODE.get());
    }

    private static float manualEv() {
        return CandelaConfig.Rt.Exposure.MANUAL_EV.value();
    }

    private static AutoConfig autoConfig() {
        return new AutoConfig(
                CandelaConfig.Rt.Exposure.KEY.value(),
                CandelaConfig.Rt.Exposure.minEv(),
                CandelaConfig.Rt.Exposure.maxEv(),
                CandelaConfig.Rt.Exposure.ADAPT_UP.value(),
                CandelaConfig.Rt.Exposure.ADAPT_DOWN.value(),
                manualEv());
    }
