package dev.comfyfluffy.candela.rt;
...
import dev.comfyfluffy.candela.client.CandelaJitter;
...
import dev.comfyfluffy.candela.rt.accel.RtAccel;
import dev.comfyfluffy.candela.rt.accel.RtBuffer;
import dev.comfyfluffy.candela.rt.accel.RtImage;
import dev.comfyfluffy.candela.rt.entity.RtEntities;
import dev.comfyfluffy.candela.rt.entity.RtEntityTextures;
import dev.comfyfluffy.candela.rt.material.RtBlockMaterials;
import dev.comfyfluffy.candela.rt.material.RtEntityMaterials;
import dev.comfyfluffy.candela.rt.pipeline.RtDisplayPipeline;
import dev.comfyfluffy.candela.rt.pipeline.RtDlssFg;
import dev.comfyfluffy.candela.rt.pipeline.RtDlssRr;
import dev.comfyfluffy.candela.rt.overlay.RtWorldOverlay;
import dev.comfyfluffy.candela.rt.pipeline.RtHdrCompositePipeline;
import dev.comfyfluffy.candela.rt.pipeline.RtSdrPresentPipeline;
import dev.comfyfluffy.candela.rt.pipeline.RtExposure;
import dev.comfyfluffy.candela.rt.pipeline.RtPipeline;
import dev.comfyfluffy.candela.rt.terrain.RtTerrain;
 ...
 * end-of-world seam. Gated by {@code -Dcandela.rt=true}.
 *
...
public final class RtComposite {
    ...
    public static boolean enabled() {
        return CandelaConfig.Rt.ENABLED.value();
    }
    ...
    private static int debugView() {
        return CandelaConfig.Rt.Composite.DEBUG_VIEW.value();
    }

    private static int spp() {
        return CandelaConfig.Rt.Composite.SPP.value();
    }

    private static int maxBounces() {
        return CandelaConfig.Rt.Composite.MAX_BOUNCES.value();
    }

    private static boolean waterWaves() {
        return CandelaConfig.Rt.Composite.WATER_WAVES.value();
    }
    ...
    private static float jitterSignX() {
        return CandelaConfig.Rt.Composite.JITTER_SIGN_X.value();
    }

    private static float jitterSignY() {
        return CandelaConfig.Rt.Composite.JITTER_SIGN_Y.value();
    }

    private static float sunNoonTilt() {
        return CandelaConfig.Rt.Composite.SUN_NOON_SOUTH_TILT.value();
    }
    ...
    /** This frame's TLAS handle (0 if none built yet), for {@code dev.comfyfluffy.candela.rt.overlay} occlusion queries. */
    ...
    public void resetFailureLatch() {
        if (failed) {
            failed = false;
            CandelaMod.LOGGER.info("RT failure latch cleared by render-state invalidation; retrying RT");
        }
    }
     ...
     * traced with — overlay raster passes ({@code dev.comfyfluffy.candela.rt.overlay}) reuse it so their content lands
    ...
    public boolean composite(GpuTexture nativeColor, int width, int height) {
        ...
        try {
            ...
            failed = true;
            CandelaMod.LOGGER.error("RT terrain streaming failed; reverting to vanilla path", t);
            return false;
        }
        ...
        try {
            ...
            if (!loggedActive) {
                loggedActive = true;
                CandelaMod.LOGGER.info("RT composite active (terrain): {}x{}, RT output replaces the world target", width, height);
            }
            return true;
        } catch (Throwable t) {
            failed = true;
            CandelaMod.LOGGER.error("RT composite failed; reverting to vanilla path", t);
            return false;
        }
    }
    ...
    public void ensureResourcesReady(RtContext ctx) {
        ...
        try {
            ...
            failed = true;
            CandelaMod.LOGGER.error("RT resource bring-up failed; reverting to vanilla path", t);
        }
    }
    ...
    private void recordFrame(RtContext ctx, RtPipeline active, GpuTexture nativeColor) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush(); RtDebugLabels.Scope frameLabel = RtDebugLabels.scope(ctx, cmd, "composite frame")) {
            ...
            float jitterY = 0f;
            if (rrPath) {
                CandelaJitter.INSTANCE.prepare(renderW, renderH, displayW);
                jitterX = CandelaJitter.INSTANCE.jitterPixelsX() * jitterSignX();
                jitterY = CandelaJitter.INSTANCE.jitterPixelsY() * jitterSignY();
            }
            ...
            try (RtDebugLabels.Scope ignored = RtDebugLabels.scope(ctx, cmd, "map RT to display");
                ...
                displayPipeline.dispatch(cmd, displayW, displayH, CandelaConfig.Rt.Hdr.enabled(),
                        CandelaConfig.Rt.Hdr.paperWhiteNits(), CandelaConfig.Rt.Hdr.headroom());
            }
            hdrWrittenThisFrame = CandelaConfig.Rt.Hdr.enabled();
            VulkanCommandEncoder.memoryBarrier(cmd, stack);
     ...
     * interpolated). {@code candela.rt.sunNoonSouthDeg} tilts the east-west arc toward south (+Z) at
     * noon.
     */
    /**
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (overlayView != 0L) {
                ...
                if (hdrCompositePipeline != null) {
                    ...
                    hdrCompositePipeline.dispatch(cmd, src.width, src.height, CandelaConfig.Rt.Hdr.paperWhiteNits());
                }
    ...
    public boolean presentSdrToPq(VulkanCommandEncoder enc, long swapchainImage, int swapW, int swapH,
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            sdrPresentPipeline.dispatch(cmd, dst.width, dst.height, CandelaConfig.Rt.Hdr.paperWhiteNits());
