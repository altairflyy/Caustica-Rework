...
public final class RtHdr {
    ...
    public static void logConfig() {
        CandelaMod.LOGGER.info(
                ...
                CandelaConfig.Rt.Hdr.enabled(),
                CandelaConfig.Rt.Hdr.PAPER_WHITE_NITS.value(), CandelaConfig.Rt.Hdr.PEAK_NITS.value(),
                CandelaConfig.Rt.Hdr.enabled() ? "HDR display path active" : "SDR display path",
                CandelaConfig.Rt.Hdr.headroom());
    }
    ...
    public static void logSurfaceCapabilities(VkPhysicalDevice phys, long surface, int chosenFormat) {
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            if (r < 0 || count.get(0) == 0) {
                CandelaMod.LOGGER.warn("HDR: could not enumerate surface formats (result={}, count={})", r, count.get(0));
                return;
            }
            int n = count.get(0);
            ...
            CandelaMod.LOGGER.info("HDR: chosen swapchain image format={} ({}); surface advertises {} (format,colorSpace) pair(s):",
            ...
            for (int i = 0; i < n; i++) {
                ...
                anyHdr |= hdr;
                CandelaMod.LOGGER.info("  [{}] format={} ({}), colorSpace={} ({}){}",
                        i, f.format(), formatName(f.format()), cs, colorSpaceName(cs), hdr ? "  <-- HDR-capable" : "");
            }
            if (anyHdr) {
                CandelaMod.LOGGER.info("HDR: at least one HDR-capable color space is exposed; PQ/HDR10 presentation is feasible once the swapchain is owned by the mod.");
            } else {
                CandelaMod.LOGGER.info("HDR: only SDR (SRGB_NONLINEAR) color spaces exposed. This is expected on a stock Minecraft Vulkan instance — VK_EXT_swapchain_colorspace must be enabled at instance-creation time (a later phase) before extended color spaces appear here.");
            }
        } catch (Throwable t) {
            CandelaMod.LOGGER.warn("HDR: surface capability enumeration failed: {}", t.toString());
        }
    }
