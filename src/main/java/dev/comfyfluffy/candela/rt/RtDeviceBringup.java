 ...
 * and the device comes up exactly as vanilla. {@code candela.rt} is read once here, at
...
public final class RtDeviceBringup {
    ...
    /**
     * Shader Execution Reordering is still required by Candela's current world raygen, but the SPIR-V
    ...
    private static boolean ommRequested() {
        return CandelaConfig.Rt.Omm.ENABLED.value();
    }

    private static boolean reflexRequested() {
        return CandelaConfig.Rt.Reflex.ENABLED.value();
    }
    ...
    public static void addFeatures(Args args, VulkanPhysicalDevice physicalDevice) {
        ...
        if (missing != null) {
            if (!loggedUnavailable) {
                ...
                CandelaMod.LOGGER.warn("Ray tracing unavailable: device [{}] lacks {}", physicalDevice.deviceName(), missing);
            }
            return;
        }
    ...
    public static void probe(VkDevice device) {
        if (!rtRequested) {
            CandelaMod.LOGGER.info("Ray tracing not requested; skipping RT probe");
        ...
        try {
            ...
            CandelaMod.LOGGER.error("RT probe threw; continuing without RT", t);
        }
    }
}
