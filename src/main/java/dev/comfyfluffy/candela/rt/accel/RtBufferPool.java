...
public final class RtBufferPool {
    ...
    /** Periodic stat dump (opt-in via {@code -Dcandela.rt.poolStats}); call once per frame. */
    public void maybeLogStats() {
        if (!CandelaConfig.Rt.BufferPool.STATS.value() || (statsCounter++ % STATS_INTERVAL) != 0) {
            return;
        }
        long freeBytes = 0;
        int freeCount = 0;
        ...
        CandelaMod.LOGGER.info("RT buffer pool: created={} reused={} | free {} buffers, {} KiB",
