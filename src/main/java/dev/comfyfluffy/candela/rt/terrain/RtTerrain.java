...
import dev.comfyfluffy.candela.mixin.SpriteContentsAccessor;
...
import dev.comfyfluffy.candela.rt.material.RtMaterials;
...
public final class RtTerrain {
    ...
    private static int asyncDispatchPerTick() {
        return CandelaConfig.Rt.Terrain.ASYNC_DISPATCH_PER_TICK.value();
    }

    private static int sectionResultsPerTick() {
        return CandelaConfig.Rt.Terrain.SECTION_RESULTS_PER_TICK.value();
    }
    ...
    private long dynamicStreamBudgetNanos() {
        float base = CandelaConfig.Rt.Terrain.STREAM_BUDGET_MS.value();
        float max = Math.max(base, CandelaConfig.Rt.Terrain.STREAM_BUDGET_MAX_MS.value());
    ...
    private static long streamFallbackBudgetNanos() {
        return (long) (CandelaConfig.Rt.Terrain.STREAM_FALLBACK_BUDGET_MS.value() * 1_000_000f);
    }
    ...
    private static int sectionTableInitialCapacity() {
        return CandelaConfig.Rt.Terrain.SECTION_TABLE_INITIAL_CAPACITY.value();
    }

    private static int rebaseDistanceBlocks() {
        return CandelaConfig.Rt.Terrain.REBASE_DISTANCE_BLOCKS.value();
    }
     ...
     * {@code candela.rt.streamBudgetMs} of wall clock so the cost is a flat slice of every frame instead
     * of a 20 TPS burst.
     */
    ...
    private static void warnMeshOnce(String what, Throwable t) {
        if (!loggedMeshFailure) {
            ...
            CandelaMod.LOGGER.warn("RT terrain: {} meshing threw (skipped); first occurrence:", what, t);
        }
    }
        ...
        while (budget > 0 && attempts-- > 0) {
            ...
            if (result.failure() != null) {
                ...
                if (!loggedTessFailure) {
                    ...
                    CandelaMod.LOGGER.warn("async terrain: tessellation task failed for section {},{},{}",
