...
import dev.comfyfluffy.candela.mixin.RenderSetupAccessor;
import dev.comfyfluffy.candela.mixin.RenderTypeAccessor;
import dev.comfyfluffy.candela.rt.material.RtEntityMaterials;
import dev.comfyfluffy.candela.rt.material.RtParallelAtlas;
...
public final class RtEntityTextures {
    /** Bindless array capacity (slot 0 reserved as a fallback texture). {@code -Dcandela.rt.maxEntityTextures}. */
    public static int maxTextures() {
        return CandelaConfig.Rt.EntityTextures.MAX_TEXTURES.value();
    }

    /** Resolve + bind per-type LabPBR {@code _n}/{@code _s} for entities. Toggle off to skip entity material
     *  branches (prim flags stay 0). {@code -Dcandela.rt.entityPbr}. */
    public static boolean entityPbr() {
        return CandelaConfig.Rt.EntityTextures.PBR.value();
    }
    ...
    public int slotForAtlas(Identifier atlasLocation) {
        ...
        try {
            ...
            if (!loggedFailure) {
                loggedFailure = true;
                CandelaMod.LOGGER.warn("RT atlas texture resolution failed for {}", atlasLocation, t);
            }
        }
    ...
    private Identifier textureLocation(RenderType renderType) {
        try {
            ...
            Object setup = ((RenderTypeAccessor) renderType).candela$state();
            Map<String, ?> textures = ((RenderSetupAccessor) setup).candela$textures();
    ...
    public long resolveView(RenderType renderType) {
        ...
        try {
            ...
            if (!loggedFailure) {
                loggedFailure = true;
                CandelaMod.LOGGER.warn("RT entity texture resolution failed for {}", renderType, t);
            }
        }
