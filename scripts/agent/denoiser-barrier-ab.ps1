...
if ($Action -eq 'Collect') {
    ...
    $markers = @(Select-String -LiteralPath $latest -Pattern "AER-083 denoiser barriers: path=$expected")
    $markers.Line | Set-Content -LiteralPath (Join-Path $run 'path-markers.txt')
    if ($markers.Count -eq 0) { throw 'Expected denoiser runtime marker missing. Exercise SVGF or NRD.' }
...
$run = Join-Path $repo ('build\rewrite-validation\AER-083-denoiser\' + $Mode + '-' + [guid]::NewGuid().ToString('N'))
...
@{mode=$Mode; startedUtc=[datetime]::UtcNow.ToString('o'); profile=$Profile; jar=$jars[0].FullName;
  ...
  generatedDenoiserBarriers=$generated; generatedPostBarriers='false'; validationRequested=$true} |
    ConvertTo-Json | Set-Content -LiteralPath (Join-Path $run 'manifest.json')
$previous = @{}
$values = @{JAVA_TOOL_OPTIONS="$env:JAVA_TOOL_OPTIONS -Dengine.renderGraphV2=true -Dengine.postBarriersV2=false -Dengine.denoiserBarriersV2=$generated";
...
Write-Host 'Launch prova and force SVGF (DLSS-RR/NRD off). Exercise first frame/reset, stationary accumulation, movement, menu and reload.'
Write-Host 'If NRD is configured, test it in a separate matched A/B pair. Exit Minecraft and Modrinth before Collect.'
